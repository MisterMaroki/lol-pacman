...
var theme_settings = [
	...
	},
	{
		...
		collection: ['assets/item_milk.png'],
	},
	{
		strokeColor: '#BA0104',
		biscuitColor: '#86C9DC',
		...
		collection: ['assets/item_heart.png'],
	},
	{
		strokeColor: '#FFE699',
		biscuitColor: '#fff',
		...
		collection: ['assets/item_wasabi.png'],
	},
	{
		strokeColor: '#F3BB0C',
		biscuitColor: '#fff',
		...
		collection: ['assets/item_pokemon.png'],
	},
	{
		strokeColor: '#666',
		biscuitColor: '#fff',
		...
		ghosts: ['assets/item_theme6_monster1.png'],
		collection: ['assets/item_gear.png'],
	},
	{
		strokeColor: '#76AA60',
		biscuitColor: '#F2A900',
...
var shareMessage =
	'[SCORE]pts is mine new highscore on Pac Man game! Try it now!'; //social share score message
...
function toggleGameMap(con) {
	if (con) {
		gameData.mapNum++;
		gameData.mapNum =
			gameData.mapNum > maps_arr.length - 1 ? 0 : gameData.mapNum;
	} else {
		gameData.mapNum--;
		gameData.mapNum =
			gameData.mapNum < 0 ? maps_arr.length - 1 : gameData.mapNum;
	}
	...
	if (
		...
		postSocketUpdate(
			'preparegame',
			{ map: gameData.mapNum, theme: gameData.themeNum },
			true
		);
	}
}
...
function toggleGameMapTheme(con) {
	if (con) {
		gameData.themeNum++;
		gameData.themeNum =
			gameData.themeNum > theme_settings.length - 1 ? 0 : gameData.themeNum;
	} else {
		gameData.themeNum--;
		gameData.themeNum =
			gameData.themeNum < 0 ? theme_settings.length - 1 : gameData.themeNum;
	}
	...
	if (checkLoop) {
		...
		if (gameData.mapLoop) {
			...
			if (maps_arr[gameData.mapNum].loop == 'horizontal') {
				gameData.mapLoopDirection =
					gameData.mapLoopSide == true ? 'left' : 'right';
			} else if (maps_arr[gameData.mapNum].loop == 'vertical') {
				gameData.mapLoopDirection =
					gameData.mapLoopSide == true ? 'up' : 'down';
			}
...
function buildMap() {
	...
	for (var h = 0; h < maps_arr[gameData.mapNum].map.length; h++) {
		...
		for (var w = 0; w < maps_arr[gameData.mapNum].map[h].length; w++) {
			...
			if (
				...
				gameData.pathArray.push({
					x: w + gameData.mapExtraX,
					y: h + gameData.mapExtraY,
				});
				gameData.totalEaten++;
			} else if (
				gameData.map[h + gameData.mapExtraY][w + gameData.mapExtraX] ==
	...
	if (
		...
		for (var n = 0; n < socketData.players.length; n++) {
			...
			cureIcon++;
			cureIcon =
				cureIcon > theme_settings[gameData.themeNum].user.length - 1
					? 0
					: cureIcon;
...
function resizeMap() {
	...
	mapContainer.scaleX = mapContainer.scaleY =
		mapScaleX < mapScaleY ? mapScaleX : mapScaleY;

	mapContainer.x =
		canvasW / 2 - (mapW / 2 - gameData.squareSize / 2) * mapContainer.scaleX;
	mapContainer.y =
		canvasH / 2 - (mapH / 2 - gameData.squareSize / 2) * mapContainer.scaleX;
	...
	if (viewport.isLandscape) {
		...
		gameLevelTxt.x =
			gameScoreTxt.x + (mapW - gameData.squareSize) * mapContainer.scaleX;
		gameLevelTxt.y = gameScoreTxt.y;

		gameLivesContainer.x = gameScoreTxt.x;
		gameLivesContainer.y =
	...
	if (viewport.isLandscape) {
		...
		$.players[1].x =
			$.players[0].x + (mapW - gameData.squareSize) * mapContainer.scaleX;
		...
		$.players[3].y =
			$.players[2].y + (mapH - gameData.squareSize + 30) * mapContainer.scaleX;
	}
}
...
function insertMapBlock(con) {
	if (
		gameData.mapLoopDirection == 'down' ||
		gameData.mapLoopDirection == 'up'
	) {
		var blockIndex = 0;
		if (con) {
...
function insertMapRow(con) {
	...
	for (
		var w = 0;
		w < maps_arr[gameData.mapNum].map[gameData.replaceIndex].length;
		w++
	) {
		gameData.map[rowIndex][w] =
			maps_arr[gameData.mapNum].map[gameData.replaceIndex][w];
		gameData.map[rowIndex][w] =
			gameData.map[rowIndex][w] == mapType.pill
				? mapType.biscuit
				: gameData.map[rowIndex][w];
	...
	if (con) {
		...
		gameData.replaceIndex =
			gameData.replaceIndex < 0
				? maps_arr[gameData.mapNum].map.length - 1
				: gameData.replaceIndex;
	} else {
		gameData.replaceIndex++;
		gameData.replaceIndex =
...
function insertMapColumn(con) {
	...
	if (
		...
		if (con) {
			...
			gameData.map[targetH][gameData.map[targetH].length - 1] =
				insertCollection();
		}
	}
...
function resetLoopMap(thisX, thisY) {
	...
	if (
		...
		gameData.mapLoopDirection == 'left' ||
		gameData.mapLoopDirection == 'right'
	) {
		for (var h = 0; h < gameData.map.length; h++) {
...
function showGameStatus(con) {
	if (con == 'ready') {
		...
		gameStatusTxt.text = textDisplay.gameReadyLevel.replace(
			'[NUMBER]',
			gameData.level + 1
		);
	} else if (con == 'clear') {
...
function drawWallShape(type) {
	...
	if (type == 'line') {
		...
		newShape.graphics
			...
			.qt(gapBold, -gapBold, gapBold, halfSquare)
			.es();
		setWallStroke(newShape);
		newShape.graphics
			.mt(-halfSquare, gapBold)
		...
		setWallStroke(newShape);
		newShape.graphics
			.mt(gapBold, -halfSquare)
			.lt(gapBold, -gapBold)
			.lt(halfSquare, -gapBold)
			.es();
		setWallStroke(newShape);
		newShape.graphics
			.mt(halfSquare, gapBold)
		...
		for (var w = 0; w < map[h].length; w++) {
			if (map[h][w] == mapType.biscuit) {
				...
				var newCollection = new createjs.Bitmap(
					loader.getResult(
						'collection_' + gameData.themeNum + '_' + collectionIndex
					)
				);
...
function insertCollection() {
	if (theme_settings[gameData.themeNum].collection.length > 0) {
			...
			if (gameData.pathArray.length > 0) {
				for (
					...
					if (gameData.map[thisY][thisX] == mapType.empty) {
						var randomCollection = Math.floor(
							Math.random() *
								theme_settings[gameData.themeNum].collection.length
						);
				...
				gameData.pathArrayIndex++;
				gameData.pathArrayIndex =
					gameData.pathArrayIndex > gameData.pathArray.length - 1
						? 0
						: gameData.pathArrayIndex;
				drawIcons();
			}
		...
		if (proceedCollection) {
			...
			var newCollection = new createjs.Bitmap(
				loader.getResult(
					'collection_' + gameData.themeNum + '_' + randomCollection
				)
			);
			centerReg(newCollection);
			...
			if (gameData.mapLoop) {
				...
				newCollection.x =
					gameData.pathArray[gameData.pathArrayIndex].x * gameData.squareSize;
				newCollection.y =
					gameData.pathArray[gameData.pathArrayIndex].y * gameData.squareSize;
...
function createPacman(icon) {
	...
	if (
		...
		nameLabel.text = textDisplay.multiplayerIndicator.replace(
			'[NUMBER]',
			gameData.users.length + 1
		);
	}
...
function createGhosts() {
	...
	for (var n = 0; n < maps_arr[gameData.mapNum].totalGhosts.length; n++) {
		...
		ghostData = new createjs.SpriteSheet({
			images: [
				loader.getResult('ghost_' + gameData.themeNum + '_' + ghostIndex).src,
			],
			frames: _frame,
			animations: _animations,
		});
		...
		ghostIndex++;
		ghostIndex =
			ghostIndex > theme_settings[gameData.themeNum].ghosts.length - 1
				? 0
				: ghostIndex;
	}
...
function resetPacman(index) {
	if (
		...
		gameData.users[index].x =
			(gameData.multiPos[gameData.multiPosIndex][0] + gameData.mapExtraX) *
			gameData.squareSize;
		gameData.users[index].y =
			(gameData.multiPos[gameData.multiPosIndex][1] + gameData.mapExtraY) *
			gameData.squareSize;
		gameData.users[index].direction =
			maps_arr[gameData.mapNum].multiDirection[gameData.multiPosIndex];
		gameData.users[index].due =
			maps_arr[gameData.mapNum].multiDirection[gameData.multiPosIndex];
		...
		gameData.multiPosIndex =
			gameData.multiPosIndex > gameData.multiPos.length - 1
				? 0
				: gameData.multiPosIndex;
	} else {
		gameData.users[index].x =
			(gameData.startPos[0] + gameData.mapExtraX) * gameData.squareSize;
		gameData.users[index].y =
			(gameData.startPos[1] + gameData.mapExtraY) * gameData.squareSize;
	...
	gameData.users[index].position = {
		x: gameData.users[index].x,
		y: gameData.users[index].y,
	};
	gameData.users[index].lastposition = {
		x: gameData.users[index].x,
		y: gameData.users[index].y,
	};
...
function movePacman(player) {
	...
	if (player.due !== player.direction) {
		...
		if (
			...
			(onGridSquare(player.position) &&
				isFloorSpace(getSquare(npos, player.due)))
		) {
	...
	if (
		onGridSquare(player.position) &&
		isWall(getSquare(npos, player.direction))
	) {
		player.direction = '';
	}
...
function moveGhosts(ghost) {
	...
	if (ghost.followPath) {
		if (
			ghost.pathArray.length > 0 &&
			ghost.pathIndex < ghost.pathArray.length
		) {
...
function pane(npos, target) {
	...
	if (checkPanX) {
		if (
			npos.x >= gameData.mapW * gameData.squareSize &&
	...
	if (checkPanY) {
		if (
			npos.y >= gameData.mapH * gameData.squareSize &&
...
function chaseUser(ghost) {
	...
	ghost.targetPath = [
		pointToCoord(ghost.followPlayer.x),
		pointToCoord(ghost.followPlayer.y),
	];
	getTargetPath(ghost);
}
...
function getNewCoord(dir, current) {
	return {
		x:
			current.x +
			((dir === 'left' && -gameData.userSpeed) ||
				(dir === 'right' && gameData.userSpeed) ||
				0),
		y:
			current.y +
			((dir === 'down' && gameData.userSpeed) ||
				(dir === 'up' && -gameData.userSpeed) ||
				0),
	};
}
...
function addScore(score, index) {
	...
	if (
		...
		var targetIndex = socketData.players.findIndex(
			(x) => x.gameIndex === index
		);
	...
	if (
		...
		for (var n = 0; n < 4; n++) {
			if (n < socketData.players.length) {
				...
				$.players['name' + n].text =
					textDisplay.multiplayerName.replace('[NUMBER]', n + 1) +
					socketData.players[n].username;
		...
		gameScoreTxt.text = textDisplay.gameScore.replace(
			'[NUMBER]',
			addCommas(playerData.score)
		);
		gameLevelTxt.text = textDisplay.gameLevel.replace(
			'[NUMBER]',
			gameData.level + 1
		);
		...
		for (var n = 0; n < gameData.collected.length; n++) {
			var newCollection = new createjs.Bitmap(
				loader.getResult(
					'collection_' + gameData.themeNum + '_' + gameData.collected[n]
				)
			);
			centerReg(newCollection);
...
function updateGame() {
	if (!gameData.paused) {
		if (timeData.enable) {
			...
			if (timeData.pillEnable) {
				timeData.elapsedTime = Math.floor(
					timeData.nowDate.getTime() - timeData.pillDate.getTime()
				);
				timeData.pillTimer = Math.floor(
					...
					if (
						...
						if (timeData.pillTimer < 5000) {
							...
							if (gameData.ghostBlink) {
								...
								gameData.ghostBlinkSide =
									gameData.ghostBlinkSide == true ? false : true;
							}
						}
		...
		for (var n = 0; n < gameData.ghosts.length; n++) {
			...
			if (
				...
				for (var p = 0; p < gameData.users.length; p++) {
					...
					var elapsedTime = Math.floor(
						nowDate.getTime() - thisGhost.ghostDate.getTime()
					);
...
function checkLoopLevel() {
	...
	if (thisLevel < mapSettings.loop.levels.length) {
		if (
			...
			for (var p = 0; p < socketData.players.length; p++) {
				if (
					socketData.players[p].score >=
					mapSettings.loop.levels[thisLevel].scoreTarget
				) {
					nextStage = true;
				}
			}
...
function getLevelSettings() {
	var thisLevel = gameData.level;
	thisLevel =
		thisLevel > mapSettings.loop.levels.length - 1
			? mapSettings.loop.levels.length - 1
			: thisLevel;
	...
	gameData.ghostEatableSpeed =
		mapSettings.loop.levels[thisLevel].ghostEatableSpeed;
}
...
function checkCollision() {
	for (var n = 0; n < gameData.ghosts.length; n++) {
		...
		for (var p = 0; p < gameData.users.length; p++) {
			...
			if (
				collided(thisPlayer, thisGhost) &&
				!thisPlayer.invisible &&
				thisPlayer.active
			) {
...
function setupTouchControl() {
	...
	for (var n = 0; n < buttonArr.length; n++) {
		...
		$.touch['arrow' + buttonArr[n]].addEventListener(
			'mousedown',
			function (evt) {
...
function toggleMoveEvent(obj, con) {
	switch (con) {
		case 'drag':
			var global = touchMoveContainer.localToGlobal(
				obj.currentTarget.x,
				obj.currentTarget.y
			);
			obj.currentTarget.offset = {
				x: global.x - obj.stageX,
				y: global.y - obj.stageY,
			};
			break;
