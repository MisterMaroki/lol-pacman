...
function buildGameCanvas() {
	...
	for (var n = 0; n < buttonArr.length; n++) {
		$.touch[buttonArr[n]] = new createjs.Bitmap(
			loader.getResult('buttonTouchPress')
		);
		centerReg($.touch[buttonArr[n]]);
		$.touch[buttonArr[n]].visible = false;

		$.touch['arrow' + buttonArr[n]] = new createjs.Bitmap(
			loader.getResult('buttonTouchArrow')
		);
		centerReg($.touch['arrow' + buttonArr[n]]);

		$.touch[buttonArr[n]].regY = $.touch['arrow' + buttonArr[n]].regY =
			$.touch[buttonArr[n]].image.naturalHeight;
		$.touch['arrow' + buttonArr[n]].rotation = $.touch[buttonArr[n]].rotation =
			rotation[n];

		createHitarea($.touch['arrow' + buttonArr[n]]);
		touchMoveContainer.addChild(
			$.touch[buttonArr[n]],
			$.touch['arrow' + buttonArr[n]]
		);
	}
