...
function buildEditButtons() {
	...
	editUser.graphics
		.beginFill(editMapData.userColor)
		.drawRect(-(thisSquare / 2), -(thisSquare / 2), thisSquare, thisSquare);
	editGhost.graphics
		.beginFill(editMapData.ghostColor)
		.drawRect(-(thisSquare / 2), -(thisSquare / 2), thisSquare, thisSquare);
	editBlock.graphics
		.setStrokeStyle(editMapData.strokeNum)
		.beginStroke(editMapData.blockStrokeColor)
		.drawRect(
...
function toggleEditOption(con, update) {
	...
	editPositionContainer.visible =
		editMultiPositionContainer.visible =
		editUser.visible =
		editGhost.visible =
			true;
...
function toggleMulti(con) {
	if (con) {
		editData.multiNum++;
		editData.multiNum =
			editData.multiNum > maps_arr[gameData.mapNum].multiPos.length - 1
				? 0
				: editData.multiNum;
	} else {
		editData.multiNum--;
		editData.multiNum =
			editData.multiNum < 0
				? maps_arr[gameData.mapNum].multiPos.length - 1
				: editData.multiNum;
	}
...
function loadMultiData() {
	...
	for (var n = 0; n < maps_arr[gameData.mapNum].ghostStayPos.length; n++) {
		...
		$.editPos['multi' + n].graphics
			.beginFill(editMapData.multiColor)
			.drawRect(-(thisSquare / 2), -(thisSquare / 2), thisSquare, thisSquare);
		...
		editMultiPositionContainer.addChild(
			$.editPos['multi' + n],
			$.editPos['multitext' + n]
		);
	}
	...
	$('#multiDirection').val(
		maps_arr[gameData.mapNum].multiDirection[editData.multiNum]
	);
	placeEditPosition();
}
...
function toggleGhost(con) {
	if (con) {
		...
		editData.ghostNum =
			editData.ghostNum < 0
				? maps_arr[gameData.mapNum].ghostStayPos.length - 1
				: editData.ghostNum;
	}
...
function loadGhostData() {
	...
	for (var n = 0; n < maps_arr[gameData.mapNum].ghostStayPos.length; n++) {
		...
		$.editPos[n].index = n;
		$.editPos[n].graphics
			.beginFill(editMapData.ghostStayColor)
			.drawRect(-(thisSquare / 2), -(thisSquare / 2), thisSquare, thisSquare);
...
function generateArray() {
	...
	for (e = 0; e < maps_arr.length; e++) {
		...
		var startPos =
			'[' + maps_arr[e].startPos[0] + ',' + maps_arr[e].startPos[1] + ']';
		var ghostPos =
			'[' + maps_arr[e].ghostPos[0] + ',' + maps_arr[e].ghostPos[1] + ']';

		var ghostStayPos = '\n';
		for (var n = 0; n < maps_arr[e].ghostStayPos.length; n++) {
			ghostStayPos += space3 + '[';
			ghostStayPos +=
				maps_arr[e].ghostStayPos[n][0] + ',' + maps_arr[e].ghostStayPos[n][1];
			ghostStayPos += '],\n';
		}
		...
		outputArray +=
			space2 + 'startDirection:"' + maps_arr[e].startDirection + '",\n';
		...
		outputArray +=
			space2 + 'totalGhosts:[' + maps_arr[e].totalGhosts.toString() + '],\n';
...
function drawEditMap() {
	...
	for (var h = 0; h < gameData.map.length; h++) {
		for (var w = 0; w < gameData.map[h].length; w++) {
			...
			$.blocks[h + '_' + w].graphics
				.setStrokeStyle(editMapData.strokeNum)
				.beginStroke(editMapData.strokeColor)
				.drawRect(
					-(gameData.squareSize / 2),
					-(gameData.squareSize / 2),
					gameData.squareSize,
			...
			$.blocks[h + '_' + w].hitArea = new createjs.Shape(
				...
				if (editData.page == 'placepoint') {
					if (editData.targetPlace != null) {
						if (editData.targetType == 'user') {
							...
							maps_arr[gameData.mapNum].ghostStayPos[editData.ghostNum][0] =
								evt.target.column;
							maps_arr[gameData.mapNum].ghostStayPos[editData.ghostNum][1] =
								evt.target.row;
						} else if (editData.targetType.substring(0, 5) == 'multi') {
							maps_arr[gameData.mapNum].multiPos[editData.multiNum][0] =
								evt.target.column;
							maps_arr[gameData.mapNum].multiPos[editData.multiNum][1] =
								evt.target.row;
						}
			...
			$.blocks[h + '_' + w].addEventListener('mouseover', function (evt) {
				...
				var wallNameIndex = editData.wallArr.findIndex(
					(x) =>
						x.value ===
						maps_arr[gameData.mapNum].map[evt.target.row][evt.target.column]
				);
				$('#blockValue').val(
					'Current block : ' +
						evt.target.column +
						...
						evt.target.row +
						' (' +
						editData.wallArr[wallNameIndex].name +
						')'
				);
	...
	editMapContainer.addChild(
		editPositionContainer,
		editMultiPositionContainer,
		editUser,
		editGhost,
		editBlock
	);
...
function placeEditPosition() {
	...
	for (var n = 0; n < maps_arr[gameData.mapNum].ghostStayPos.length; n++) {
		if ($.editPos[n] != undefined) {
			$.editPos[n].alpha = 0.7;
			$.editPos[n].x =
				maps_arr[gameData.mapNum].ghostStayPos[n][0] * gameData.squareSize;
			$.editPos[n].y =
				maps_arr[gameData.mapNum].ghostStayPos[n][1] * gameData.squareSize;
	...
	for (var n = 0; n < maps_arr[gameData.mapNum].multiPos.length; n++) {
		if ($.editPos['multi' + n] != undefined) {
			$.editPos['multi' + n].alpha = 0.7;
			$.editPos['multi' + n].x =
				maps_arr[gameData.mapNum].multiPos[n][0] * gameData.squareSize;
			$.editPos['multi' + n].y =
				maps_arr[gameData.mapNum].multiPos[n][1] * gameData.squareSize;
...
function switchBlock(column, row) {
	...
	var wallArrIndex = editData.wallArr.findIndex(
		(x) => x.value === thisWallType
	);

	if ($('input.replaceBlock').is(':checked')) {
		gameData.map[row][column] = Number($('#wallist option:selected').val());
		maps_arr[gameData.mapNum].map[row][column] = Number(
			$('#wallist option:selected').val()
		);
	} else {
		if (wallArrIndex != -1) {
			if (!editData.backward) {
				wallArrIndex++;
				wallArrIndex =
					wallArrIndex > editData.wallArr.length - 1 ? 0 : wallArrIndex;
			} else {
				wallArrIndex--;
				wallArrIndex =
					wallArrIndex < 0 ? editData.wallArr.length - 1 : wallArrIndex;
			}
			gameData.map[row][column] = editData.wallArr[wallArrIndex].value;
			maps_arr[gameData.mapNum].map[row][column] =
				editData.wallArr[wallArrIndex].value;
		}
	}
...
function toggleWallSelect(con) {
	...
	if (con) {
		selectedIndex++;
		selectedIndex =
			selectedIndex > editData.wallArr.length - 1 ? 0 : selectedIndex;
	} else {
		selectedIndex--;
