...
function initPreload() {
	...
	for (var n = 0; n < theme_settings.length; n++) {
		...
		for (var f = 0; f < theme_settings[n].collection.length; f++) {
			manifest.push({
				src: theme_settings[n].collection[f],
				id: 'collection_' + n + '_' + f,
			});
		}
	}
	...
	if (soundOn) {
		...
		manifest.push({
			src: 'assets/sounds/sound_collect.ogg',
			id: 'soundCollect',
		});
		manifest.push({
			src: 'assets/sounds/sound_countdown.ogg',
			id: 'soundCountdown',
		});
		manifest.push({ src: 'assets/sounds/sound_dead.ogg', id: 'soundDead' });
		manifest.push({
			src: 'assets/sounds/sound_eat_ghost.ogg',
			id: 'soundEatGhost',
		});
		manifest.push({
			src: 'assets/sounds/sound_eat_pill.ogg',
			id: 'soundEatPill',
		});
